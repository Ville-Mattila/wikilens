...
**Chrome / Edge / Brave / Vivaldi:** install from the [Chrome Web Store](https://chromewebstore.google.com/detail/wikilens/ipjonmoniljhdbjdplfcocdfjlbifcck) in one click.

**Firefox:** install from [Firefox Add-ons](https://addons.mozilla.org/en-US/firefox/addon/wikilens/) in one click.
...
Click the gear in the toolbar popup, or right-click the WikiLens icon → **Options** (opens in its own tab). Settings sync via `chrome.storage.sync` and apply immediately, with no reload needed:
